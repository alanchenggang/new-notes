...
1. 由于很多人(当然也包括本人), 容易搞混 `不可重复读` 和 `幻读`, 这两者确实非常相似。
   ...
   - 而 `幻读` 主要是说多次读取一个范围内的记录(包括直接查询所有记录结果或者做聚合统计), 发现结果不一致(标准档案一般指记录增多, 记录的减少应该也算是幻读)。(可以参考[MySQL官方文档对 Phantom Rows 的介绍](https://link.segmentfault.com/?enc=AK9KJr1JAwshWSYUvVBy1w%3D%3D.jbtHEC13U9ETbrQq8GkEcHNDqtcXaEZJR68bruCpxTyw1eOtJOq%2BeBdAttxgAiYazeRU5sx3C1a7OpkVqwISNNBR0KmyEWAq2%2Bw8jVjyVvE%3D))
